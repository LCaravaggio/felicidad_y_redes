...
local varlist fel sat lavg_d_mbps edad remoto ingresos estudios capital
matrix A = J(8,5,0)
local i=1
...
local varlist fel sat SNU lavg_d_mbps edad remoto ingresos estudios capital
matrix B = J(9,4,0)
local i=1
foreach var of local varlist {
    quietly sum `var'
...
local varlist fel SNU lavg_d_mbps edad remoto ingresos estudios capital
matrix C = J(8,4,0)
local i=1
foreach var of local varlist {
...
quietly reg sat SNU ingresos edad remoto, robust
estimates store A
outreg2 using "Irrestricto 2020.tex", tex(frag) ctitle("MCO") replace fmt(fc) dec(3)  nor2

*First Stage
quietly reg SNU lavg_d_mbps ingresos edad remoto, robust
estimates store B
...
quietly ivreg2 sat estudios edad remoto (SNU=lavg_d_mbps) , robust first
estimates store D
...
estimates store E
outreg2 using "Irrestricto 2020.tex", tex(frag) ctitle("sin remoto") append fmt(fc) dec(3) nor2

quietly ivreg2 sat ingresos edad remoto capital (SNU=lavg_d_mbps) , robust first
estimates store E
